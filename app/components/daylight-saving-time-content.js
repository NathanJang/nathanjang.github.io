...
export default class DaylightSavingTimeContentComponent extends Component {
  ...
  get currentSunsetPDT() {
    const dayOfYear = Math.floor((this.timerService.currentDate - new Date(this.timerService.currentDate.getFullYear(), 0, 0)) / (1000 * 60 * 60 * 24));
    ...
    if (dayOfYear <= dayOfYearMin || dayOfYear > dayOfYearMax) {
      ...
      const resultantMinutesAfterMidnight = -amplitude * Math.cos(frequency * (dayOfYear - phaseShift)) + midline;
      const hours = this._zeroPad(Math.floor(resultantMinutesAfterMidnight / 60));
      const minutes = this._zeroPad(Math.floor(resultantMinutesAfterMidnight % 60));
  ...
  get currentSunsetPST() {
    if (this.isOnSummerTime) { return this.currentSunsetPDT; }
